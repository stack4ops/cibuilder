if [ "${pipeline_env}" = "local" ] && [ "${local_only}" = "1" ]; then
  log 2 "skip minortag.check in local_only mode"
  return 18
fi
...
get_minor_tag() {
  log 2 "start: get_minor_tag"

  if [ ! -n "${minor_tag_regex:-}" ]; then
    log 1 "no minor tag regex defined. skipping check_minor_tag"
    return 0
  fi
  
  if ! repository_tags=$(skopeo inspect $creds $tls_verify "docker://$base_image:$base_tag"); then
    log 0 "something went wrong while inspecting the image docker://$base_image:$base_tag"
    return 1
  fi
  ...
  if ! minor_tags=$(echo "$repository_tags" | jq --arg minor_tag_regex_arg "${minor_tag_regex}" '.RepoTags | reverse | .[] | select(. | test($minor_tag_regex_arg))'); then
    log 0 "something went wrong while parsing the minor tags from the inspected json result"
    return 1
  fi
  for minor_tag in $minor_tags; do
    ...
    if ! minor_tag_digest=$(skopeo inspect $creds --no-tags --format '{{ .Layers }}' $tls_verify docker://${base_image}:$mt | tr -d '"' | tr -d '[:space:]'); then
      log 0 "something went wrong while getting the minor_tag_digest for docker://${base_image}:$mt"
    fi
    log 2 "$minor_tag_digest - $mt"
    if [ "$minor_tag_digest" = "$current_digest" ]; then
      ...
      log 2 "cache_minor_tag: ${cache_minor_tag}"
      return 18
      break
    fi
  done
